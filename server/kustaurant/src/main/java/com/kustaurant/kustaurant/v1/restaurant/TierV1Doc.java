...
@Tag(
        name = "restaurant v1 tier controller",
        description = "구 앤드포인트 호환용 컨트롤러 :: v2작업시 필요할 수 있어 남겨둡니다."
)
public interface TierV1Doc {
    ...
    ResponseEntity<List<RestaurantTierDTO>> getTierChartList(
    ...
    ResponseEntity<List<RestaurantTierDTO>> getTierChartListWithAuth(
            @Parameter(hidden = true) @ChartCond ChartCondition condition,
            @Parameter(hidden = true) @AuthUser AuthUserInfo user
    );
    ...
    ResponseEntity<RestaurantTierMapDTO> getMapInfoWithAuth(
            @Parameter(hidden = true) @ChartCond ChartCondition condition,
            @Parameter(hidden = true) @AuthUser AuthUserInfo user
    );
}
