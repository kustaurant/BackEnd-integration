...
function initMap(mapData) {
  ...
  var favoriteIds = favoriteRestaurants.map(
...
// [[ 지도 열기 버튼 클릭 시 ]]
  ...
  let isMapOpen = false;
  document.getElementById('mapOpenButton').addEventListener('click',
      function () {
        isMapOpen = true;
        history.pushState({}, '');
...
// [[ 지도 닫기 버튼 클릭 시 ]]
  document.getElementById('mapCloseButton').addEventListener('click',
      function () {
        history.back();
      })
...
// 1. 즐찾 마커/정보창 생성
  ...
  for (const restaurant of favoriteRestaurants) {
    let marker = new naver.maps.Marker({
      position: new naver.maps.LatLng(restaurant.latitude,
...
// 즐찾 식당 마커
  attachClickHandlers(favoriteRestaurantMarkers,
      favoriteRestaurantInfoWindows);
// 티어 없는 식당 마커
