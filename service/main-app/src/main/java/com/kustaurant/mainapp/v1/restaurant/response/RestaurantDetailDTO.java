...
public class RestaurantDetailDTO {
    ...
    public static RestaurantDetailDTO fromV2(RestaurantDetail detail) {
        return new RestaurantDetailDTO(
                (int) (long) detail.getRestaurantId(),
                detail.getRestaurantImgUrl(),
                detail.getMainTier(),
                detail.getRestaurantCuisine(),
                detail.getRestaurantCuisineImgUrl(),
                detail.getRestaurantPosition(),
                detail.getRestaurantName(),
                detail.getRestaurantAddress(),
                detail.getIsOpen(),
                detail.getBusinessHours(),
                detail.getNaverMapUrl(),
                detail.getSituationList(),
                detail.getPartnershipInfo(),
                detail.getEvaluationCount(),
                detail.getRestaurantScore(),
                detail.getIsEvaluated(),
                detail.getIsFavorite(),
                detail.getFavoriteCount(),
                detail.getRestaurantMenuList().stream().map(RestaurantMenu::fromV2).toList()
        );
    }
}
