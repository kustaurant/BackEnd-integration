...
import com.kustaurant.kustaurant.evaluation.comment.controller.response.EvalCommentReactResponse;
...
public class EvalCommApiController {
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "댓글 리스트입니다.", content = {@Content(mediaType = "application/json", array = @ArraySchema(schema = @Schema(implementation = EvalCommentResponse.class)))}),
    ...
    public ResponseEntity<List<EvalCommentResponse>> getReviewList(
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "리뷰 추천하기 누르고 난 후의 추천 수를 반환해줍니다.", content = {@Content(mediaType = "application/json", schema = @Schema(implementation = EvalCommentResponse.class))}),
    ...
    public ResponseEntity<EvalCommentReactResponse> likeComment(
            @PathVariable Integer restaurantId,
        ...
        if (!isSubComment(commentId)) { // 평가 댓글인 경우
            ...
            return ResponseEntity.ok(new EvalCommentReactResponse(1,1,1,1));
        } else { // 대댓글인 경우
    ...
    @Operation(summary = "리뷰 비추천하기", description = "비추천을 누른 후의 추천수와 비추천수를 반환합니다.\n\n" +
            ...
            "   - commentReplies: **null입니다.**")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "리뷰 비추천하기 누르고 난 후의 비추천 수를 반환해줍니다.", content = {@Content(mediaType = "application/json", schema = @Schema(implementation = EvalCommentResponse.class))}),
    ...
    public ResponseEntity<EvalCommentReactResponse> dislikeComment(
            @PathVariable Integer restaurantId,
            @PathVariable Integer commentId,
            @Parameter(hidden = true) @AuthUser AuthUserInfo user
    ) {
    ...
    @Operation(summary = "리뷰 대댓글 달기", description = "작성한 대댓글을 반환합니다.\n\n" +
            ...
            "   - commentReplies: **null입니다.**")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "작성한 대댓글을 반환합니다.", content = {@Content(mediaType = "application/json", schema = @Schema(implementation = EvalCommentResponse.class))}),
    ...
    public ResponseEntity<Void> postReply(
        ...
        // 대댓글 달기
        EvalCommentEntity restaurantComment = restaurantCommentService.addSubComment(restaurant, user.id(), commentBody, evaluation);
...
//        return new ResponseEntity<>(EvalCommResponse.convertCommentWhenSubComment(restaurantComment, null, user.id()), HttpStatus.OK);
    ...
    public ResponseEntity<Void> deleteComment(
        ...
        if (isSubComment(commentId)) { // 대댓글인 경우
            ...
            checkRestaurantIdAndCommentId(restaurant, restaurantId, commentId);
            // 댓글 가져오기
            EvalCommentEntity comment = restaurantCommentService.findCommentByCommentId(commentId);
            // 댓글 삭제
