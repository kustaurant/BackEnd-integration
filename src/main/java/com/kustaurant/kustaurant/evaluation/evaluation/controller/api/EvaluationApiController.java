...
public class EvaluationApiController {
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "평가하기 후에 식당 정보를 다시 반환해줍니다.", content = {@Content(mediaType = "application/json", array = @ArraySchema(schema = @Schema(implementation = EvalCommentResponse.class)))}),
    ...
    public ResponseEntity<List<EvalCommentResponse>> evaluateRestaurant(
            @PathVariable Integer restaurantId,
        ...
        // TODO: 일단 주석처리함.
//        return ResponseEntity.ok(restaurantCommentService.getRestaurantCommentList(restaurantId, user.id(), true));
