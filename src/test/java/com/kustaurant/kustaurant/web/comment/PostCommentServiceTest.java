...
class PostCommentServiceTest {
    ...
    void 댓글_좋아요_처음_누르면_좋아요가_생성된다() {
        ...
        when(likeRepository.existsByUser_UserIdAndPostComment_CommentId(userId, commentId)).thenReturn(false);
        when(dislikeRepository.existsByUser_UserIdAndPostComment_CommentId(userId, commentId)).thenReturn(false);
    ...
    void 댓글_싫어요에서_좋아요로_전환시_싫어요_감소_좋아요_증가한다() {
        ...
        when(commentRepository.findById(commentId)).thenReturn(Optional.of(comment));
        when(likeRepository.existsByUser_UserIdAndPostComment_CommentId(userId, commentId)).thenReturn(false);
        when(dislikeRepository.existsByUser_UserIdAndPostComment_CommentId(userId, commentId)).thenReturn(true);
